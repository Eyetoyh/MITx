...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    for i in range(num_samples):
        xi = feature_matrix[i]
        yi = labels[i]
        margin = yi * (np.dot(theta, xi) + theta_0)
        loss = max(0, 1 - margin)
        total_loss += loss
...
def pegasos_single_step_update(
    ...
    if label * (np.dot(current_theta, feature_vector) + current_theta_0) <= 1:
        """
        This additional if-else loop: the avoid np.dot missing the agreement 'b'
        """
        if len(current_theta.shape) == 1:
            updated_theta = (1 - eta * L) * current_theta + eta * label * feature_vector
        else:
            updated_theta = (1 - eta * L) * current_theta + eta * label * feature_vector.reshape((-1,1))
...
def classifier_accuracy(
    ...
    # Compute the accuracy on the training and validation data
